...
// DESIGN.md 8 — Auth mekanizması: local kullanıcılar için email+password login,
// JWT bearer token. LDAP login (ldapjs bind) henüz uygulanmadı.
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user.authSource !== AuthSource.local) {
      return res.status(501).json({
        error: "ldap_login_not_supported",
        message: "LDAP login henüz uygulanmadı",
      });
    }
    ...
    res.json({ token, user: await userProfile(user.id) });
  }),
);
